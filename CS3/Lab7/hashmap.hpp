...
public:
   ...
   pair<const pair<const Key, Value>*, bool> insert(const Element& x);    // inserts the key/value pair 
   pair<const pair<const Key, Value>*, bool> erase(const Key& x);         // erases element with key x, if exists
...
template <typename Key, typename Value, typename Compare, typename Hash>
pair<const pair<const Key, Value>*, bool> hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
...
template <typename Key, typename Value, typename Compare, typename Hash>
pair<const pair<const Key, Value>*, bool> hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {

   size_t bucket = hash_.hash(x);
   auto it = findElement(x, bucket);    // try to find the element

   if(it == elems_[bucket].end()) { // element does not exist
   ...
   // find next bucket with elements and point iterator there
   while(next == elems_[bucket].end() && bucket < elems_.size() - 1) {
      ++bucket;
      next = elems_[bucket].begin();
   }

   const pair<const Key, Value>* np;

   // if it is pointer to end of bucket, then no more valid elements exist
   // otherwise, point to next valid element
   ...
   // remove element from hashmap and decrement size
   elems_[bucket].erase(it);
   --size_;

   // return pair<const pair<const Key, Value>*, bool> 
   // where bool is true since erase was successful
   return make_pair(np, true);
}

template <typename Key, typename Value, typename Compare, typename Hash>
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
   // if n is less than current number of buckets, return
   if(n < hash_.numBuckets()) return;

   // create new vector of buckets size n
   vector<list<Element>> buckets(n);
   ...
   for(auto& bucket : elems_) {
      for(auto& e : bucket) {
         size_t newBucket = hash_.hash(e.first) % n;
         buckets[newBucket].push_back(e);
      }
   }

   elems_ = std::move(buckets);
}
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
   ...
   // if x is not in hashmap
   if (found == nullptr) { 
      auto insertResult = insert(make_pair(x, Value())); 
      // cast pointer since can point to constant element
      found = const_cast<Element*>(insertResult.first); 
   }
   return found->second;
}